...
use Abills::Base qw(_bp load_pmodule encode_base64);
...
sub proccess {
  ...
  if ($self->{conf}{PAYSYS_LIQPAY_OLD} && $self->{conf}{PAYSYS_LIQPAY_OLD} == 1) {
    ...
    my $data = $FORM->{data};
    my $json_data = Abills::Base::decode_base64($data);
...
sub user_portal_old_style {
  ...
  if($attr->{UNTOKEN} == 1){
    ...
    my $user_token_del = $Paysys->paysys_user_info({
      UID          => $user->{UID},
    });

    $Paysys->paysys_main_del({
      order_id => "$user_token_del->{ORDER_ID}"
    });
  ...
  if($self->{conf}{PAYSYS_LIQPAY_SUBSCRIBE}){
  ...
  elsif($self->{conf}{PAYSYS_LIQPAY_SUBSCRIBE_TOKEN}){
    if (! $user->{PHONE}) {
      $html->message( 'err', "$self->{lang}->{ERROR}", "$self->{lang}->{ERR_MESSAGE}" );
    }
    ...
    my $list_token_info = $Paysys->paysys_user_info({
      UID          => $user->{UID},
    });

    if(defined $list_token_info->{TOKEN}){
      $info{SUM} = sprintf("%.2f", $list_token_info->{SUM});
      $info{DATE} = POSIX::strftime('%Y-%m-%d', ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst));
      my $url_env = ($ENV) ? "$ENV{PROT}://$ENV{SERVER_NAME}:$ENV{SERVER_PORT}" : $attr->{IP};
      $info{HREF} = "$url_env/index.cgi?index=$attr->{index}&PAYMENT_SYSTEM=$attr->{PAYMENT_SYSTEM}&SUM=1.00&UNTOKEN=1";
      ...
      if (!$attr->{CHECKBOX}) {
        ...
        $html->tpl_show(main::_include('paysys_liqpay_add_rec', 'Paysys'), {
          %$attr,
          SUBSCRIBE_DATE_START => "$day_start",
          DESCRIPTION          => $description,
          PHONE                => $user->{PHONE},
        ...
        if($attr->{checkbox} == 1) {
          ($info{SIGN}, $info{BODY}) = $self->liqpay_make_request_token({
            DOMAIN_ID   => $user->{DOMAIN_ID},
            GID         => $user->{GID},
            DESCRIPTION => $description,
            SUM         => sprintf(".2f", $attr->{SUM}),
...
sub cnb_form {
  ...
  my $data = Abills::Base::encode_base64($json_payment);

  $data =~ s/[\r\n]+//g;

  my $sign_string;

  if(defined $self->{conf}{PAYSYS_LIQPAY_MERCHANT_PASS}) {
    $sign_string = $self->{conf}{PAYSYS_LIQPAY_MERCHANT_PASS} . $data . $self->{conf}{PAYSYS_LIQPAY_MERCHANT_PASS};
  }
  else{
    $sign_string = $self->{db}{PAYSYS_LIQPAY_MERCHANT_PASS} . $data . $self->{db}{PAYSYS_LIQPAY_MERCHANT_PASS};
  }
  use Digest::SHA1 qw/sha1_base64 sha1/;
  my $signature = Abills::Base::encode_base64(Digest::SHA1::sha1($sign_string));
  $signature =~ s/[\r\n]+//g;
...
sub str_to_sign {
  ...
  use Digest::SHA1 qw/sha1_base64 sha1/;
  my $signature = Abills::Base::encode_base64(Digest::SHA1::sha1($str));
  chop($signature);
...
sub _process_old_syle {
  ...
  my $data = $FORM->{data};
  my $result_hash = JSON::decode_json(Abills::Base::decode_base64($data));
  ...
  my $card_token = $result_hash->{card_token} || q{};
  my $paytype = $result_hash->{paytype} || q{};
  ...
  if($status eq 'subscribed'){
    ...
    print $paysys_status;
    if(defined $card_token && $paytype eq 'card'){
     $Paysys->paysys_user_add({
     UID    => $list->[0]->{uid},
     TOKEN  => $card_token,
     SUM    => $result_hash->{'amount'},
...
sub liqpay_make_request_subscribe {
  ...
  my ($sign, $body, $form) = $self->cnb_form({
    'version'     => '3',
    'public_key'  => $self->{conf}{PAYSYS_LIQPAY_MERCHANT_ID},
    'action'      => 'subscribe',
    'amount'      => $attr->{'amount'} || $attr->{TOTAL_SUM},
...
#**********************************************************
=head2 liqpay_make_request_subscribe1()
...
=cut
#**********************************************************
sub liqpay_make_request_subscribe1 {
  my $self = shift;
  my ($attr) = @_;
...
sub liqpay_subscribe_update {
 ...
 if ($attr->{DATA}) {
    my $sign = $self->str_to_sign(
      $self->{db}{PAYSYS_LIQPAY_MERCHANT_PASS} . $attr->{DATA} . $self->{db}{PAYSYS_LIQPAY_MERCHANT_PASS}
    );

    return $sign;
  }

  my $server_url = $self->{db}{PAYSYS_LIQPAY_URL};

  my ($sign, $body, $form) = $self->cnb_form({
    'version'     => '3',
    'public_key'  => $self->{db}{PAYSYS_LIQPAY_MERCHANT_ID},
    'action'      => 'paytoken',
    'amount'      => $attr->{SUM},
    'order_id'    => $attr->{ORDER_ID_TOKEN},
    'currency'    => $attr->{'currency'} || $self->{db}{'PAYSYS_LIQPAY_CURRENCY'} || 'UAH',
    'description' => "Pay paytoken",
    'card_token'  => $attr->{CARD_TOKEN},
    'server_url'  => $server_url
  }
  );

  return $sign, $form, $body;
}
#**********************************************************
...
sub paysys_subscribe_pay{
  my $self = shift;
  my ($attr) = @_;
  my ($uid, $sum, $order_id, $token) = @_;

  my $Paysys = Paysys->new($self->{conf}, $self->{admin}, $self->{db});
  ...
  require Abills::Misc;
  my $recomended_pay = $user_object->{deposit} || 0;

  my $gid = $user_object->{gid};
  my $abon2 = abs($recomended_pay);
  my $order_id_token = "Liqpay:" . mk_unique_value(8, { SYMBOLS => '0123456789'});
  my $commission_sum;
  my $amount_pay;

  if ($self->{db}{PAYSYS_LIQPAY_COMMISSION}) {
    $self->{db}{PAYSYS_LIQPAY_COMMISSION} =~ /([0-9\.]+)([\%]?)/;
    my $commision = $1;
    my $type = $2;

    if ($type) {
      $commission_sum = sprintf("%.2f",
        ($abon2 + ($abon2 / 100 * $commision)) / 100 * $commision);
      $commission_sum = int($commission_sum * 100);
      $commission_sum = ($commission_sum + 1) / 100;
    }
    else {
      $commission_sum = sprintf("%.2f", $commision);
    }
  }
  ...
  if ($recomended_pay > 0) {
    ...
    $amount_pay = sprintf("%.2f", ($abon2 + $commission_sum));
